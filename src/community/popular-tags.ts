 ...
 * 방어 처리: tags 컬럼이 아직 DB에 없을 수 있으므로 입력을 unknown으로 받아
 * 각 행의 tags를 런타임에 확인한다 (없거나 배열이 아니면 건너뛴다).
 */
export const topTags = (rows: readonly unknown[], limit: number): string[] => {
  // 빈도 + 첫 등장 순서를 함께 기록 (동률 tie-break용)
  const count = new Map<string, number>();
  const firstSeen = new Map<string, number>();
  let order = 0;
  for (const row of rows) {
    const tags = (row as { tags?: unknown } | null)?.tags;
