 ...
 * 인접 흰건반 경계에 겹쳐 배치한다. 노트 입력은 레코딩으로만 —
 * 평소 pointerdown은 프리뷰 사운드(onKeyTap), 레코딩 중(recording)에는
 * pointer down/up으로 누른 길이를 캡처한다 (piano-recording-design §3.3).
...
export interface PianoKeysProps {
  ...
  /** 레코딩 중 — true면 프리뷰 대신 pointer down/up으로 연주 캡처 */
...
export const PianoKeys = ({
  ...
  const handlers = (p: number) =>
    recording
      ...
      : { onPointerDown: (): void => onKeyTap(asMidi(p)) };
