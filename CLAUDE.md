...
- URL 해시 인코딩은 무점 `v2<base64url(바이너리)>` — 해시에 `.` 금지, URL 총 500자 이내 유지
  (메신저 링크 감지기가 `.`에서 절단하고 500자 초과 URL은 링크화하지 않음). 디코딩은 v2 + 무점 v1(JSON)만.
  구형 `v1.` 해시는 하위 호환 폐기(2026-07-15). 잘리거나 깨진 해시·미래 버전 해시는 조용히 깨지지 말고 명시적으로 거부.
- 게시된 릭의 공유 URL은 단축 라우트 `/s/:id` — 열면 자체 완결 열람 URL(`/?mode=view#v2…`)로 replace.
